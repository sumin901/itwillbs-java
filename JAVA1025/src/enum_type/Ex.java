...
public class Ex {
	...
	public static void main(String[] args) {
		...
		mc.setMyWeek(EnumWeek.WEDNSEDAY);
		System.out.println("오늘의 요일 : " + mc.getMyWeek());
		
		// 열거 타입 사용 시 장점
		// 1. 정의 시 지정된 상수 외의 다른값은 전달 불가능, 강제성(예외가없다)
//		mc.setMyWeek(1); 
//		>> 오류 : The method setMyWeek(EnumWeek) in the type MyCalendar 지정된 값만 넣어라(강제성)
//		mc.setMyWeek("WEDNSEDAY");
//		mc.setMyWeek(WEDNSEDAY);
//		>> 오류! 반드시 열거타입명.상수명으로 지정된 값만 전달해야한다
		mc.setMyWeek(EnumWeek.WEDNSEDAY);
...
// 열거타입을 사용하는 클래스 
class MyCalendar {
	EnumWeek myWeek; // 클래스 내에서 열거타입 변수 선언(로컬변수도 가능)
